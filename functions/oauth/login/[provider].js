...
export async function onRequest(context) {
    ...
    try {
        ...
        const nonce = provider === 'google' ? crypto.randomUUID() : null;
        ...
        await env.DB.prepare(
            `INSERT INTO oauth_transactions (tx, provider, state, code_verifier, nonce) VALUES (?, ?, ?, ?, ?)`
        ).bind(tx, provider, state, pkce.verifier, nonce).run();
